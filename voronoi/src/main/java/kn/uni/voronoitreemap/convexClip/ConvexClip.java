...
public class ConvexClip {
	...
	public void Start(cVertexList list1, cVertexList list2) {
		...
		if(!isConvex(p)){ //Check for convexity
			...
			if(!isConvex(p)){
				System.out.println("Polygons are not Convex...1");
				System.out.println(p.toString());
				throw new RuntimeException("Polygons are not Convex...2");
			}
		}		
		if(!isConvex(q)){ //Check for convexity
			q.ReverseList();//If list is not oriented counterclockwise
			if(!isConvex(q)){
				System.out.println("Polygons are not Convex...3");
				System.out.println(q.toString());
				throw new RuntimeException("Polygons are not Convex...4");
			}
		}
