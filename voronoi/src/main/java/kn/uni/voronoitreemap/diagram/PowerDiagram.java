...
public class PowerDiagram {
	 ...
	 */
	public Vector<Site> computeDiagram() {
...
//					s.setWeight(0.001);
		...
		} else if (sites.size == 1) { // case added to deal with only one site. The polygon associated to one site is the bounding box
			sites.get(0).setPolygon(clipPoly);
		} else {
			throw new RuntimeException("no sites are set");
		}

		Vector<Site> nonEmptySites = new Vector<Site>();
		for (int i = 0; i < sites.size; i++) {
			if (sites.array[i] != null && sites.array[i].getPolygon() != null) nonEmptySites.add(sites.array[i]);
		}

		return nonEmptySites;
	}
...
//							try {
                            ...
                            //System.out.println(site.getY());
                            //System.out.println(site.nonClippedPolyon.getNumPoints());
